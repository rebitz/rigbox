...
if sum(strcmp(files,filestart))<1 % but if we're wrong
    ...
    if walkRewards;
        [t1Rwd,t2Rwd,t3Rwd] = generateWalkForBandit3(hazard,nToGen ,[rwdLB rwdUB]);
    else % randomly assign the two bounds to the values
        [t1Rwd,t2Rwd,t3Rwd] = generateBlockForBandit3(hazard, nToGen ,[rwdLB rwdUB], minContinuousValues);
    end
...
task_data.t1orientation = t1orientation;
task_data.t2orientation = t2orientation;
task_data.t3orientation = t3orientation;
