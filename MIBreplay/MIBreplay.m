...
function [task_data, trial_data] = MIBreplay(filename, mode)

try
    
    ListenChar(2);

    MIBreplay_params;    % Load the parameters
    openTask; % general rig specific
    MIBreplay_opentask;  % specific open task func
    
    % Run the task
    while trialnum < ntrials && continue_running
                
        MIBreplay_opentrial; % Initialize a new trial
        MIBreplay_runtrial;  % Run the trial
        
        MIBreplay_closetrial;    % Close the trial
