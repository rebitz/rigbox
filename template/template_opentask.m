...
G = fspecial('gaussian', vhSize, gSigma*1.5);
G = G ./ (max(max(G))*(ones(vhSize))); % normalize
% make a noise stimulus; brown = -2; pink = -1; white = 0
tmp = spatialPattern(vhSize,-1);
tmp = (tmp-min(min(tmp)))./(max(max(tmp))-min(min(tmp)));
% combine the two, rescale; 
maskIm = (G.*tmp+mean(mean(tmp))-G/2).*env.colorDepth;
% also rescale again
maskIm = maskIm+((env.colorDepth/2)-median(median(maskIm)));
% sca; ListenChar(0); keyboard();
mskIndx = Screen('MakeTexture', w, maskIm);
